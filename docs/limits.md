...
| `platforms/facebook.py`, `CAUSES` and the failure messages the write check builds | what a Graph API refusal means and what to do about it, and what a write that could not be proved to be a reply did | you, in your terminal |
| `approve.py`, the halt summary | that the queue was stopped part way rather than finished, and that the last audit line is marked unverified | you, in your terminal |
